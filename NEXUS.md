...
| ⚠️ A status word must name what it measures, not what it sounds like (2026-08-13) | Intelligence-UI / zero-fabrication tasks: `staleness_seconds` is **feature-data recency**, not match state, yet `freshnessLabel()` rendered its freshest tier as `"Live"` on every scheduled fixture — and the endpoint behind that list only ever returns `status == "scheduled"`, so the badge was *structurally incapable* of being true. Now `Fresh`/`Recent`/`Stale`. **Routing rule: before rendering any state word (`Live`, `Verified`, `Ready`, `Active`), name the field behind it and ask whether that field can even express the thing the word claims.** The backend has a real `match.status` (`scheduled`/`live`/`finished`) that no fixture-card component reads — use it if a genuine match-state badge is ever wanted. ⚠️ Related, same session: `readiness-ring.tsx`'s "Predictions verified" was *not* fabricated (real capability probe) but sat beside `CERTIFICATION: UNVERIFIED`; it now reads "Prediction **pipeline** verified". **Pipeline capability, model certification, provider activation, and infra readiness are four different axes — never let one's copy imply another.** |
| ⚠️ Grid/flex items default to `min-width: auto` — the classic overflow trap (2026-08-13) | Any responsive/layout task: a 27px horizontal page scroll on every mobile viewport traced to the fixture row `<a>` being a **grid item**, which defaults to `min-width: auto` (min-content) and so rendered 359px inside its own 303px column. `min-w-0` was present on the row's *inner* text block but not on the row itself — the inner one does not help; the binding constraint is on the item in the grid/flex container. **Routing rule: when a child must shrink, `min-w-0` belongs on the item that is the grid/flex child, and `truncate` alone is not sufficient.** Diagnose by measuring, not by theory: apply the candidate fix via `page.evaluate` and re-measure `documentElement.scrollWidth` before editing source — theorising about min-content wasted several passes here. Load `frontend-product-design-architect` + `component-quality-gate`. |
| Route metadata: the brand belongs to the root layout only (2026-08-13) | Any task adding or editing a page's `metadata.title`: `app/layout.tsx` declares `template: "%s \| Sabiscore"`, so a page that spells the brand into its own `title:` renders it twice in the tab, the SEO `<title>`, and every social card. This shipped on **all 9** titled routes at once, so a point-fix would not have prevented the next. Machine-enforced by `apps/web/src/lib/metadata-title-contract.test.ts` — if it fails on your change, drop the brand from the page, do not relax the assertion. |
...
| ⚠️ A manifest field that nothing reads is not a contract (Phase 3, 2026-08-21) | ML/model, release-ops and settlement tasks: `active_generation.json` hash-protects every artifact but `feature_schema_version` was **unvalidated free text** until this session — the identical asymmetry `_verify_certification_claim` already closed for the promotion verdict, one field over. It is now resolved against `FEATURE_SCHEMA_VERSIONS` (`models/feature_registry.py`) and cross-checked against each league's hash-verified metadata `feature_count`. **Routing rule: when adding any field to a manifest, an artifact metadata file, or a generation record, name its reader in the same change — a declared-but-unread field reads as a guarantee while enforcing nothing.** Two independent forces made this one invisible: six consumers republish the string as provenance (`/health`, `/api/v1/models/status`, `legacy_endpoints`, every prediction), and `prediction.py` answers a width mismatch with `_fallback_result()` rather than raising, so a relabel degrades silently rather than crashing. ⚠️ Also surfaced: `scripts/verify_active_artifacts.py` loads `active_generation.py` **standalone by file path** so the Render build gate never pulls in the app import chain (importing `src.models` opens a DB connection). A relative import there breaks the deploy and **no test catches it** — run the gate script itself after touching that module's imports. Route to `sabiscore-settlement-calibration-architect` + `backend-systems-auditor`. |
| ⚠️ A derived artifact's freshness gate belongs at build time, not on the startup path (Phase 3, 2026-08-21) | ML/model and release-ops tasks: `backend/models/feature_contract.json` is generated from `feature_registry.build_feature_contract()` and guarded by `verify_feature_contract_freshness()` — called from `scripts/verify_active_artifacts.py` (Render buildCommand), deliberately **not** from `load_active_generation()`. That loader runs at startup (`_startup_load_models_strict`) and on the settlement/staking path (`active_model_version`, `active_generation_is_certified`), so coupling it to a derived *documentation* file means a forgotten regeneration crash-loops a running service over metadata — the vΩ.47 shape. At build time instead, drift fails the deploy and the previous release keeps serving. **Routing rule: when adding a generated/derived artifact with a drift check, ask which paths its validator will now run on. If the answer includes startup or a request path, the check belongs in the build gate instead.** The first draft of this change wired it into the loader; two `test_certification_integrity.py` failures surfaced the coupling. ⚠️ Related anti-fabrication rule: `build_feature_contract()` marks the 14 §7.1 fields no code can answer as the literal `UNDECLARED`, pinned by `test_unanswerable_fields_are_literally_undeclared`. **If that test fails on your change, supply a real derivation and remove the field from `_UNDECLARED_FIELDS` — never hand-write a plausible value to make it pass.** Route to `sabiscore-settlement-calibration-architect` + `backend-systems-auditor`. |
| ⚠️ One bad record can wedge an entire batched background job — check every loop-with-a-single-commit for this shape (2026-08-16) | Any task touching a background sync/settlement loop (`elo_state_service.sync_elo_from_finished_matches`, `fixture_sync_service.sync_upcoming_fixtures`, and by extension any future one with the same structure): a `for` loop that calls a function which can raise, with the loop's only `session.commit()` sitting *after* the loop, means one bad record anywhere in the batch discards every good record already processed in that same tick — not a slow degradation, a permanent zero-progress wedge, because the same poison record is re-selected and re-hit every subsequent tick. Found and fixed twice in one session: Elo sync (`elo_rating_snapshots` stuck at 0 rows — 26 self-play `matches` rows, `docs/DEBT.md` item 23) and fixture sync (`ValueError` on a rescheduled fixture dropping the whole tick's fixtures, `docs/DEBT.md` item 24). **Routing rule: when reviewing or extending a batch-processing loop, wrap the individual risky call in a per-record `try/except`, log + increment a metric, and `continue` — never let one record's failure discard the whole batch's progress.** Route to `backend-systems-auditor` + `testing-strategy-architect`; a regression test should seed one bad record alongside a good one in the same batch and assert the good one still commits. |
