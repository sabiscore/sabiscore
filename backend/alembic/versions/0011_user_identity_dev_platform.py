...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "user_id", "entity_type", "entity_id", name="uq_user_favorites_user_entity"
        ),
    )
    op.create_index("ix_user_favorites_user_id", "user_favorites", ["user_id"])
    op.create_index(
        "ix_user_favorites_anon_id", "user_favorites", ["anonymous_session_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.UniqueConstraint(
            "user_id", "match_id", name="uq_user_saved_matches_user_match"
        ),
    )
    op.create_index("ix_user_saved_matches_user_id", "user_saved_matches", ["user_id"])
    op.create_index(
        "ix_user_saved_matches_anon_id", "user_saved_matches", ["anonymous_session_id"]
    )
    op.create_index(
        "ix_user_saved_matches_match_id", "user_saved_matches", ["match_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "timezone", sa.String(), nullable=False, server_default="Africa/Lagos"
        ),
    ...
    op.create_index(
        "ix_user_preferences_anon_id", "user_preferences", ["anonymous_session_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "rate_limit_per_minute", sa.Integer(), nullable=False, server_default="10"
        ),
    ...
    op.create_index(
        "ix_analytics_events_anon_id", "analytics_events", ["anonymous_session_id"]
    )
    ...
    op.create_index(
        "ix_notif_subs_user_id", "user_notification_subscriptions", ["user_id"]
    )
    op.create_index(
        "ix_notif_subs_anon_id",
        "user_notification_subscriptions",
        ["anonymous_session_id"],
    )
    op.create_index(
        "ix_notif_subs_match_id", "user_notification_subscriptions", ["match_id"]
    )
    ...
    op.create_index(
        "ix_notif_logs_user_unread",
        "user_notification_logs",
        ["user_id", "read", "created_at"],
    )
    op.create_index(
        "ix_notif_logs_anon_unread",
        "user_notification_logs",
        ["anonymous_session_id", "read", "created_at"],
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_notif_subs_match_id", table_name="user_notification_subscriptions"
    )
