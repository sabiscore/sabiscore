...
def devig(odds: tuple[float, float, float]) -> np.ndarray:
    implied = np.asarray([1.0 / x for x in odds], dtype=np.float32)
    total = float(implied.sum())
    if not math.isfinite(total) or total <= 0:
        raise ValueError("invalid implied market probability")
    return (implied / total).astype(np.float32, copy=False)


def market_probs(row: dict[str, Any]) -> tuple[np.ndarray, str] | None:
