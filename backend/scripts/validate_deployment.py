...
def get_json(url: str, *, timeout: float, token: str | None = None) -> dict[str, Any]:
    headers = {
        ...
        "Pragma": "no-cache",
        "User-Agent": "sabiscore-g24-validator/2",
    }
    if token:
...
def vercel_deployment(payload: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    source = (
        deployment.get("gitSource")
        if isinstance(deployment.get("gitSource"), dict)
        else {}
    )
    for value in (
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    def read() -> dict[str, Any]:
        deployment_payload = get_json(
            deployments_url, timeout=args.timeout, token=token
        )
        deployment_sha, deployment = vercel_deployment(deployment_payload)
        if str(deployment.get("readyState") or "").upper() != "READY":
            raise RuntimeError(
                f"latest production deployment is not READY: {deployment.get('readyState')!r}"
            )
    ...
    errors: list[str] = []
    for label, actual in (
        ("vercel_deployment_sha", deployment_sha),
        ("frontend_vercelSha", frontend_vercel),
        ("frontend_backendSha", frontend_backend),
        ("render_release_sha", backend_release),
    ):
        if actual != expected:
            errors.append(f"{label}: expected {expected}, observed {actual}")
    if frontend_backend != backend_release:
        errors.append(
            f"frontend/backend SHA mismatch: {frontend_backend} != {backend_release}"
        )
    for label, payload in (("frontend", frontend), ("backend", backend)):
        status = str(payload.get("status") or "").lower()
        if status not in {"ok", "healthy", "ready"}:
            errors.append(
                f"{label} health status is not healthy: {payload.get('status')!r}"
            )
    return {
        "report_version": "v7.3-harness-2",
        "generated_at": datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "repository": {"name": "sabiscore/sabiscore", "commit_sha": expected},
        "deployment": {
            "backend_sha": backend_release,
            "frontend_sha": deployment_sha,
            "schema_version": frontend.get("schemaVersion"),
            "migration_revision": backend.get("migration_revision"),
            "compatibility_status": "PASS" if not errors else "FAIL",
            "vercel_ready_state": observed["deployment"].get("readyState"),
        },
        "model": {"status": "NOT_EVALUATED"},
        "data": {"provenance_status": "NOT_EVALUATED"},
        "policy": {"metric_convention": "exact deployment identity parity"},
        "metrics": {
            "deployment_parity": {
        ...
        "operator_gates": {},
        "risks": [
            "G24 proves release identity and health-contract parity, not functional correctness of the deployed application."
        ],
        "changes": [],
        "validation": {
            "executed": True,
            "cache_busting": True,
            "backoff_seconds": {"base": args.base_delay, "max": args.max_delay},
        },
...
def main() -> int:
    ...
    try:
        ...
        result = {
            ...
            "gates": {"G24": {"status": "BLOCKED", "errors": [str(exc)]}},
            "decision": "BLOCKED",
        }
    out = Path(
        os.getenv("SABISCORE_G24_OUTPUT", "artifacts/certification/g24_deployment.json")
    )
