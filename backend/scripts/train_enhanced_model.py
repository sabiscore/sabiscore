...
async def main():
    ...
    if not data_path.exists() or force_download:
        async with EnhancedDataPipeline() as pipeline:
            ...
            df = await pipeline.download_all_historical_data(
                leagues=config["leagues"], seasons=config["seasons"]
            )
            ...
            odds_cols = [
                c for c in df.columns if "pinnacle" in c.lower() or "odds" in c.lower()
            ]
            if odds_cols:
    ...
    evaluation = evaluate_model_realistically(model, X_test, y_test, odds_test)
