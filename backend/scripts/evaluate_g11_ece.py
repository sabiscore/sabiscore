...
def load(path: Path) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    ...
    if len(targets) < 50:
        raise RuntimeError(
            f"G11/G15 requires >=50 valid forecasts; found {len(targets)}"
        )
    return (
        np.asarray(probs, dtype=np.float32),
        np.asarray(targets, dtype=np.int8),
        {"leagues": sorted(leagues), "generation_ids": sorted(generations)},
    )
...
def adaptive_confidence_ece(
    probs: np.ndarray, y: np.ndarray, n_bins: int
    ...
    return {
        "estimator": "adaptive_equal_mass_confidence",
...
def classwise_ece(probs: np.ndarray, y: np.ndarray, n_bins: int) -> dict[str, float]:
    ...
    for c, name in enumerate(CLASS_NAMES):
        ...
        out[name] = float(
            sum(
                (len(idx) / len(y))
                * abs(float(probs[idx, c].mean()) - float(event[idx].mean()))
                for idx in bins
            )
        )
    return out
...
def murphy_brier(
    probs: np.ndarray, y: np.ndarray, bins: list[np.ndarray]
) -> dict[str, float]:
    ...
    brier = float(
        np.mean(
            np.sum((probs.astype(np.float64) - one_hot.astype(np.float64)) ** 2, axis=1)
        )
    )
    ...
    for idx in bins:
        ...
        cov = np.sum((probs[idx] - p_bar) * (one_hot[idx] - o_bar), axis=1).mean(
            dtype=np.float64
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Adaptive equal-mass ECE + Murphy Brier evidence"
    )
    ...
    try:
        ...
        if abs(brier["decomposition_error"]) > 1e-6:
            raise RuntimeError(
                f"Murphy decomposition identity failed: {brier['decomposition_error']}"
            )
        result = {
            ...
            "model": {
                ...
                "generation_id": provenance["generation_ids"],
            },
            "data": {
                "dataset_snapshot": str(args.input),
                "snapshot_sha256": sha256_file(args.input),
                "coverage": {"n": int(len(y)), "leagues": provenance["leagues"]},
                "provenance_status": "INPUT_ARTIFACT_REQUIRED",
                "forecast_authenticity": "REQUIRES_PREMATCH_FORECAST_ARTIFACT",
            },
            "policy": {
                "metric_convention": "adaptive equal-mass confidence ECE; multiclass Murphy decomposition"
            },
            ...
            "gates": {
                "G11": {
                    "status": "MEASURED",
                    "value": ece["ece"],
                    "criterion": "certification_policy threshold applied by compiler",
                },
                "G15": {
                    "status": "MEASURED",
                    "value": brier["decomposition_error"],
                    "criterion": "Murphy identity residual <= 1e-6",
                },
            },
            "operator_gates": {},
            "risks": [
                "This artifact measures calibration; the compiler owns gate aggregation."
            ],
            "changes": [],
            "validation": {
                "executed": True,
                "binning": "stable equal-mass",
                "memory_strategy": "float32 + explicit gc",
            },
            "decision": "MEASURED",
        }
        ...
        result = {
            ...
            "repository": {"name": "sabiscore/sabiscore", "commit_sha": git_sha()},
            "gates": {
                "G11": {"status": "BLOCKED", "errors": [str(exc)]},
                "G15": {"status": "BLOCKED", "errors": [str(exc)]},
            },
            "decision": "BLOCKED",
        }
    out = (
        ...
        / f"g11_ece_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}.json"
    )
