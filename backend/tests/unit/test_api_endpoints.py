...
def test_health_check(test_client: TestClient) -> None:
    ...
    with (
        patch("src.api.endpoints.monitoring.get_engine", return_value=mock_engine),
        patch("src.core.cache.cache.ping", return_value=True),
        patch(
            "src.core.cache.cache.metrics_snapshot",
            return_value={"hits": 10, "misses": 1},
        ),
    ):
    ...
    try:
        ...
        with (
            ...
            response = test_client.get(
                f"/api/v1/matches/search?q={query_text}&league={league}&limit={limit}"
            )
