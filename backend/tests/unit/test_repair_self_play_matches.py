...
async def test_repairs_a_known_self_play_row(session: AsyncSession, tmp_path: Path):
    ...
    session.add(
        Team(id="fd-team-serie_a:ac_milan", name="AC Milan", league_id="SERIE_A")
    )
    session.add(
        Team(
    ...
    corrupted_id = historical_match_id(
        "SERIE_A", datetime(2019, 9, 21), "Milan", "Inter"
    )
    session.add(
        Match(
            id=corrupted_id,
    ...
    report = await find_and_repair_self_play_matches(
        session, cache_dir=tmp_path, apply=True
    )
    ...
    fixed = (
        await session.execute(select(Match).where(Match.id == corrupted_id))
    ).scalar_one()
...
async def test_dry_run_reports_without_mutating(session: AsyncSession, tmp_path: Path):
    ...
    report = await find_and_repair_self_play_matches(
        session, cache_dir=tmp_path, apply=False
    )
    assert report.repaired == 1
    ...
    assert (
        unchanged.home_team_id
        == unchanged.away_team_id
        == "fd-team-serie_a:fc_internazionale_milano"
    )


async def test_skips_row_whose_original_csv_is_unavailable(
...
async def test_skips_row_that_still_collides_after_reresolution(
    session: AsyncSession, tmp_path: Path
):
    ...
    corrupted_id = historical_match_id(
        "LA_LIGA", datetime(2020, 1, 1), "Real Madrid", "Real Madrid"
    )
    session.add(
        Match(
            id=corrupted_id,
