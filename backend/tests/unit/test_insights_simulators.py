...
def test_full_simulation_produces_a_coherent_outcome_simplex() -> None:
    simulator = MatchSimulator(random_seed=42)
    result = simulator.run_match_simulation(
        home_xg=1.6, away_xg=1.1, n_sims=500, max_goals=8
    )
    ...
    total = sum(
        float(outcomes[k]) for k in ("home_win_prob", "draw_prob", "away_win_prob")
    )
...
def test_simulation_is_reproducible_for_a_fixed_seed() -> None:
    first = MatchSimulator(random_seed=11).run_match_simulation(
        1.4, 1.2, n_sims=300, max_goals=6
    )
    second = MatchSimulator(random_seed=11).run_match_simulation(
        1.4, 1.2, n_sims=300, max_goals=6
    )
