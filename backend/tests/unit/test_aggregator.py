...
def test_aggregator_with_live_data():
    ...
    with (
        patch("src.data.aggregator.FlashscoreScraper") as MockFlashscore,
...
def test_aggregator_fallback_to_cache():
    ...
    with (
        patch("src.data.aggregator.cache.get", return_value=cached_data),
...
def test_aggregator_error_handling():
    ...
    with patch("src.data.aggregator.FlashscoreScraper") as MockFlashscore:
        MockFlashscore.return_value.scrape_match_results.side_effect = Exception(
            "Test error"
        )
