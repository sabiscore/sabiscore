...
def test_startup_priming_preserves_the_meta_model_for_calibrated_serving() -> None:
    ...
    # Use a class name that is intentionally absent from _META_MODEL_CALIBRATION_LABELS
    # so the engine honestly reports calibration_method="none" / calibration_applied=False.
    # The test's intent is to verify the meta_model object is preserved end-to-end,
    # not to validate calibration label lookup — that is covered by test_model_artifact_loading.py.
    class RawStackingHead:
        def predict_proba(self, X: np.ndarray) -> np.ndarray:
            assert X.shape == (1, 3)  # one learner -> 3 meta-feature columns
            return np.array([[0.70, 0.10, 0.20]])

    meta_model = RawStackingHead()
    ...
    # RawStackingHead is not in _META_MODEL_CALIBRATION_LABELS, so it is honestly uncalibrated
