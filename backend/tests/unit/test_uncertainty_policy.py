...
def test_specification_reports_its_real_validation_outcome():
    ...
    assert set(IMPLEMENTATION_STATUS["gates_passed"]) == set(UNCERTAINTY_GATES) - {
        "error_association"
    }
    assert (
        IMPLEMENTATION_STATUS["gate_remains_closed"] == "MODEL_UNCERTAINTY_UNAVAILABLE"
    )
...
def test_policy_is_deep_copied_so_it_cannot_be_mutated_in_process():
    ...
    assert uncertainty_policy_sha256() == before, (
        "policy digest moved after caller mutation"
    )
    assert (
...
def test_digest_is_stable_and_content_addressed():
    ...
    assert (
        uncertainty_policy_sha256(uncertainty_policy()) == uncertainty_policy_sha256()
    )
