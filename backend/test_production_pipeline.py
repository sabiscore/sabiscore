...
class PipelineValidator:
            ...
            try:
                ...
                if name == "flashscore":
                    data = scraper.fetch_data(
                        home_team="Arsenal", away_team="Chelsea", use_cache=True
                    )
                    ...
                    data = scraper.fetch_data(
                        team="Arsenal", league="EPL", use_cache=True
                    )
                elif name == "whoscored":
                    data = scraper.fetch_data(league="EPL", use_cache=True)
                elif name == "soccerway":
                    data = scraper.fetch_data(league="EPL", use_cache=True)
                elif name == "transfermarkt":
                    data = scraper.fetch_data(team="Arsenal", use_cache=True)
                elif name == "football_data":
                    data = scraper.download_season_data(
                        league="EPL", season="2324", use_cache=True
                    )

                self.results["scrapers"][name] = {
                    "status": "✅ PASS",
                    "data_available": data is not None
                    and (len(data) > 0 if hasattr(data, "__len__") else True),
                }
                logger.info(f"  ✅ {name}: OK")

            except Exception as e:
                self.results["scrapers"][name] = {"status": "❌ FAIL", "error": str(e)}
    ...
    async def test_aggregator(self):
        ...
        try:
            ...
            if features:
                ...
                logger.info(
                    f"  Feature prefixes: {set(k.split('_')[0] for k in features.keys())}"
                )
    ...
    async def test_prediction_service(self):
        ...
        try:
            ...
            prediction = await service.predict_match(
                match_id="test_arsenal_chelsea", request=request
            )
            ...
            logger.info(
                f"  Processing: {prediction.metadata.get('processing_time_ms', 0)}ms"
            )
            ...
            self.results["prediction"]["processing_ms"] = prediction.metadata.get(
    ...
    async def test_metrics(self):
        ...
        try:
            ...
            if "predictions" in summary:
                ...
                logger.info(
                    f"  Cache hit rate: {pred_metrics.get('cache_hit_rate', 0):.1%}"
                )
                logger.info(
                    f"  Avg latency: {pred_metrics.get('avg_latency_ms', 0):.1f}ms"
                )
            ...
            if "scrapers" in summary:
                ...
                for scraper, stats in summary["scrapers"].items():
                    logger.info(
                        f"    {scraper}: {stats.get('success_rate', 0):.1%} success"
                    )
    ...
    def print_summary(self):
        ...
        all_pass = (
            all(
                r["status"] in ["✅ PASS", "⚠️ WARN"]
                for r in self.results["scrapers"].values()
            )
            and self.results["aggregator"].get("status", "").startswith("✅")
            and self.results["prediction"].get("status", "").startswith(("✅", "⚠️"))
            and self.results["metrics"].get("status", "").startswith("✅")
        )
