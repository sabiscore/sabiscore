...
_MarketEvent = tuple[tuple[str, str], list[dict[str, Any]]]


def _event_team_names(event: _MarketEvent) -> tuple[str, str]:
...
class OddsService:
    ...
    async def get_match_odds(
        ...
        usable = [
            ...
            if record.get("coherent") and record.get("executable")
        ]
        # Resolve to exactly one provider event before reading any price. The
        # board carries one record per bookmaker, so uniqueness is judged over
        # distinct events -- several bookmakers quoting the same fixture is the
        # normal case, two different fixtures matching the same name is not.
        events: dict[tuple[str, str], list[dict[str, Any]]] = {}
        for record in usable:
            events.setdefault(
                (str(record.get("home_team", "")), str(record.get("away_team", ""))), []
            ).append(record)

        board: list[_MarketEvent] = list(events.items())
        resolved = select_unique_by_team_names(
            board,
            home=home_team,
            away=away_team,
            league=competition,
        ...
        matched, ambiguous = resolved
        if ambiguous:
            logger.warning(
                "Ambiguous live-odds identity for %s vs %s; refusing to guess",
                home_team,
                away_team,
            )
