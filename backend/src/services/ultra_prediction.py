...
class UltraPredictionService:
    ...
    async def predict_match(
        ...
        value_bets = self._detect_value_bets(
            match_id, probabilities, request.odds, bankroll
        )
    ...
    def _build_features_from_request(
        self, request: MatchPredictionRequest
    ) -> Dict[str, float]:
        ...
        if request.home_form_last_5:
            features["home_last_5_wins"] = sum(
                1 for f in request.home_form_last_5 if f == 3
            )
            features["home_last_5_draws"] = sum(
                1 for f in request.home_form_last_5 if f == 1
            )
            features["home_last_5_losses"] = sum(
                1 for f in request.home_form_last_5 if f == 0
            )
        else:
        ...
        if request.away_form_last_5:
            features["away_last_5_wins"] = sum(
                1 for f in request.away_form_last_5 if f == 3
            )
            features["away_last_5_draws"] = sum(
                1 for f in request.away_form_last_5 if f == 1
            )
            features["away_last_5_losses"] = sum(
                1 for f in request.away_form_last_5 if f == 0
            )
        else:
        ...
        features["home_goals_conceded_avg"] = features.get(
            "home_goals_conceded_avg", 1.2
        )
        features["away_goals_conceded_avg"] = features.get(
    ...
    def _generate_explanations(
        self, features: Dict[str, float], result: Dict[str, Any]
    ) -> Dict[str, Any]:
