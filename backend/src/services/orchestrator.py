...
class ProductionOrchestrator:
    ...
    def predict(
        ...
        try:
            ...
            features = {}
            if db:
                features = self.data_processing.get_match_features(db, match_id)
            ...
            prediction = self.model_orchestrator.predict(
                league=league, match_data=match_data, odds=odds
            )
    ...
    def batch_predict(
        self, matches: List[Dict], db: Optional[Session] = None
    ) -> List[Dict]:
        """
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "last_prediction": self.metrics.last_prediction.isoformat()
                if self.metrics.last_prediction
                else None,
            },
        }
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_scrape": self.metrics.last_scrape.isoformat()
            if self.metrics.last_scrape
            else None,
            "last_prediction": self.metrics.last_prediction.isoformat()
            if self.metrics.last_prediction
            else None,
    ...
    def _calculate_confidence(self, prediction: Dict, features: Dict) -> float:
        ...
        if predictions:
            max_prob = (
                max(predictions.values()) if isinstance(predictions, dict) else 0.33
            )
    ...
    async def _health_check_loop(self):
        ...
        while self._running:
            try:
                ...
                self.redis.setex(
                    "pipeline:health", self._health_check_interval, json.dumps(health)
                )
