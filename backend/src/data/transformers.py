...
class FeatureTransformer:
    ...
    def _validate_required_evidence(self, match_data: Dict[str, Any]) -> None:
        ...
        if not all(
            self._has_numeric(odds, key) and float(odds[key]) > 1.0
        ...
        if missing:
            raise DataUnavailableError(
                "Required feature evidence unavailable: "
    ...
    def engineer_features(self, match_data: Dict[str, Any]) -> pd.DataFrame:
        ...
        historical_stats = match_data.get("historical_stats")
        if historical_stats is None or (
            isinstance(historical_stats, pd.DataFrame) and historical_stats.empty
        ):
        ...
        head_to_head = match_data.get("head_to_head")
        if head_to_head is None or (
            isinstance(head_to_head, pd.DataFrame) and head_to_head.empty
        ):
    ...
    def _project_to_canonical_features(
        self, features: pd.DataFrame, match_data: Dict[str, Any]
    ) -> pd.DataFrame:
        ...
        derive_market = (
            derive_apex_market_features
            if is_apex_schema(self.schema_version)
        ...
        canonical["h2h_dominance"] = (
            canonical["h2h_home_wins"] - canonical["h2h_away_wins"]
        ) / canonical["h2h_matches"]
        ...
        canonical["form_market_agreement_home"] = (
            canonical["home_form_last5_home"] / 3.0
        ) * market_prob_home
        canonical["form_market_disagreement"] = abs(
            (canonical["home_form_last5_home"] / 3.0) - market_prob_home
        )
        canonical.update(
            derive_combination_features(
        ...
        canonical["venue_market_combo"] = (
            canonical["home_venue_win_rate"] * market_prob_home
        )
        canonical["h2h_market_agreement"] = (
            canonical["h2h_dominance"] * market_prob_home
        )
        ...
        if "elo_home_trend_5" in expected or "elo_momentum_cross" in expected:
            elo_home_trend_5 = _source_num(
                "elo_home_trend_5",
                row.get("home_elo_trend_5"),
                home_stats.get("elo_trend_5"),
            )

        elo_away_trend_5: Optional[float] = None
        if "elo_away_trend_5" in expected or "elo_momentum_cross" in expected:
            elo_away_trend_5 = _source_num(
                "elo_away_trend_5",
                row.get("away_elo_trend_5"),
                away_stats.get("elo_trend_5"),
            )
        ...
        if (
            "elo_momentum_cross" in expected
            and elo_home_trend_5 is not None
            and elo_away_trend_5 is not None
        ):
    ...
    def _create_base_features(self, historical_stats: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if not last_5.empty:
                ...
                features["home_goals_conceded_per_match_5"] = last_5[
                    "away_score"
                ].mean()
                features["away_goals_conceded_per_match_5"] = last_5[
                    "home_score"
                ].mean()
                ...
                # GD stats
                features["home_gd_avg_5"] = (
                    last_5["home_score"] - last_5["away_score"]
                ).mean()
                features["away_gd_avg_5"] = (
                    last_5["away_score"] - last_5["home_score"]
                ).mean()
    ...
    def _add_form_features(
        self, features: pd.DataFrame, current_form: Dict[str, Any]
    ) -> pd.DataFrame:
        for side in ("home", "away"):
            ...
            features[f"{side}_form_5"] = (
                points / 15.0 if results else self._legacy_default(f"{side}_form_5")
            )
            ...
            features[f"{side}_form_10"] = form.get(
                "form_10", features[f"{side}_form_5"]
            )
            features[f"{side}_form_20"] = form.get(
                "form_20", features[f"{side}_form_10"]
            )

            # Streaks
            features[f"{side}_win_streak"] = self._value_or_legacy(
                form, "win_streak", f"{side}_win_streak"
            )
            features[f"{side}_unbeaten_streak"] = self._value_or_legacy(
                form, "unbeaten_streak", f"{side}_unbeaten_streak"
            )

        return features

    def _add_odds_features(
        self, features: pd.DataFrame, odds: Dict[str, float]
    ) -> pd.DataFrame:
        from ..core.config import (
            settings,
        )  # local import avoids circular at module load
        ...
        has_1x2 = bool(
            odds.get("home_win") and odds.get("draw") and odds.get("away_win")
        )
        if not has_1x2:
            if self._fail_closed_enabled() and getattr(
                settings, "provider_fail_closed", True
            ):
        ...
        features["odds_volatility_1h"] = self._value_or_legacy(
            odds, "volatility_1h", "odds_volatility_1h"
        )
        features["market_panic_score"] = self._value_or_legacy(
            odds, "panic_score", "market_panic_score"
        )
        features["odds_drift_home"] = self._value_or_legacy(
            odds, "drift_home", "odds_drift_home"
        )

        return features

    def _add_injury_features(
        self, features: pd.DataFrame, injuries: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _add_h2h_features(
        self, features: pd.DataFrame, head_to_head: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _add_team_stats_features(
        self, features: pd.DataFrame, team_stats: Dict[str, Any]
    ) -> pd.DataFrame:
        home_stats = team_stats.get("home", {})
        away_stats = team_stats.get("away", {})

        # Squad value features
        features["home_squad_value"] = self._value_or_legacy(
            home_stats, "squad_value", "home_squad_value"
        )
        features["away_squad_value"] = self._value_or_legacy(
            away_stats, "squad_value", "away_squad_value"
        )
        features["home_missing_value"] = self._value_or_legacy(
            home_stats, "missing_value", "home_missing_value"
        )
        features["away_missing_value"] = self._value_or_legacy(
            away_stats, "missing_value", "away_missing_value"
        )
        features["squad_value_diff"] = (
            features["home_squad_value"] - features["away_squad_value"]
        )
    ...
    def _add_advanced_team_features(
        self, features: pd.DataFrame, team_stats: Dict[str, Any]
    ) -> pd.DataFrame:
        home_stats = team_stats.get("home", {})
        away_stats = team_stats.get("away", {})
        ...
        for side, stats in [("home", home_stats), ("away", away_stats)]:
            features[f"{side}_xg_avg_5"] = self._value_or_legacy(
                stats, "xg_avg_5", f"{side}_xg_avg_5"
            )
            features[f"{side}_xg_conceded_avg_5"] = self._value_or_legacy(
                stats, "xg_conceded_avg_5", f"{side}_xg_conceded_avg_5"
            )
            features[f"{side}_xg_diff_5"] = self._value_or_legacy(
                stats, "xg_diff_5", f"{side}_xg_diff_5"
            )
            features[f"{side}_xg_overperformance"] = self._value_or_legacy(
                stats, "xg_overperformance", f"{side}_xg_overperformance"
            )
            features[f"{side}_xg_consistency"] = self._value_or_legacy(
                stats, "xg_consistency", f"{side}_xg_consistency"
            )

        features["xg_differential"] = (
            features["home_xg_diff_5"] - features["away_xg_diff_5"]
        )

        # Tactical features
        for side, stats in [("home", home_stats), ("away", away_stats)]:
            features[f"{side}_possession_style"] = self._value_or_legacy(
                stats, "possession_style", f"{side}_possession_style"
            )
            features[f"{side}_pressing_intensity"] = self._value_or_legacy(
                stats, "pressing_intensity", f"{side}_pressing_intensity"
            )
            features[f"{side}_first_half_goals_rate"] = self._value_or_legacy(
                stats, "first_half_goals_rate", f"{side}_first_half_goals_rate"
            )
            features[f"{side}_defensive_solidity"] = self._value_or_legacy(
                stats, "defensive_solidity", f"{side}_defensive_solidity"
            )
            features[f"{side}_setpiece_goals_rate"] = self._value_or_legacy(
                stats, "setpiece_goals_rate", f"{side}_setpiece_goals_rate"
            )

            # GD trends
            features[f"{side}_gd_trend"] = self._value_or_legacy(
                stats, "gd_trend", f"{side}_gd_trend"
            )
            features[f"{side}_scoring_consistency"] = self._value_or_legacy(
                stats, "scoring_consistency", f"{side}_scoring_consistency"
            )

        return features

    def _add_form_trends(
        self, features: pd.DataFrame, current_form: Dict[str, Any]
    ) -> pd.DataFrame:
        ...
        # Momentum features
        features["home_momentum_lambda"] = self._value_or_legacy(
            home_form, "momentum_lambda", "home_momentum_lambda"
        )
        features["home_momentum_weighted"] = self._value_or_legacy(
            home_form, "momentum_weighted", "home_momentum_weighted"
        )
        features["away_momentum_lambda"] = self._value_or_legacy(
            away_form, "momentum_lambda", "away_momentum_lambda"
        )
        features["away_momentum_weighted"] = self._value_or_legacy(
            away_form, "momentum_weighted", "away_momentum_weighted"
        )

        # Fatigue features
        features["home_days_rest"] = self._value_or_legacy(
            home_form, "days_rest", "home_days_rest"
        )
        features["home_fatigue_index"] = self._value_or_legacy(
            home_form, "fatigue_index", "home_fatigue_index"
        )
        features["home_fixtures_14d"] = self._value_or_legacy(
            home_form, "fixtures_14d", "home_fixtures_14d"
        )
        features["home_fixture_congestion"] = self._value_or_legacy(
            home_form, "fixture_congestion", "home_fixture_congestion"
        )

        features["away_days_rest"] = self._value_or_legacy(
            away_form, "days_rest", "away_days_rest"
        )
        features["away_fatigue_index"] = self._value_or_legacy(
            away_form, "fatigue_index", "away_fatigue_index"
        )
        features["away_fixtures_14d"] = self._value_or_legacy(
            away_form, "fixtures_14d", "away_fixtures_14d"
        )
        features["away_fixture_congestion"] = self._value_or_legacy(
            away_form, "fixture_congestion", "away_fixture_congestion"
        )
    ...
    def _add_schedule_features(
        ...
        features["temperature"] = self._value_or_legacy(
            weather, "temperature", "temperature"
        )
        features["precipitation"] = self._value_or_legacy(
            weather, "precipitation", "precipitation"
        )
        features["wind_speed"] = self._value_or_legacy(
            weather, "wind_speed", "wind_speed"
        )
        features["weather_impact_score"] = self._value_or_legacy(
            weather, "impact_score", "weather_impact_score"
        )

        # Home advantage features
        features["home_advantage_win_rate"] = self._value_or_legacy(
            schedule, "home_advantage_win_rate", "home_advantage_win_rate"
        )
        features["home_goals_advantage"] = self._value_or_legacy(
            schedule, "home_goals_advantage", "home_goals_advantage"
        )
        features["away_win_rate_away"] = self._value_or_legacy(
            schedule, "away_win_rate_away", "away_win_rate_away"
        )
        features["home_crowd_boost"] = self._value_or_legacy(
            schedule, "home_crowd_boost", "home_crowd_boost"
        )
        features["home_advantage_coefficient"] = self._value_or_legacy(
            schedule, "home_advantage_coefficient", "home_advantage_coefficient"
        )
        features["referee_home_bias"] = self._value_or_legacy(
            schedule, "referee_home_bias", "referee_home_bias"
        )
    ...
    def _apply_enhanced_features(
        ...
        enhanced = (
            match_data.get("enhanced_features")
        ...
        overrides: tuple[
            tuple[
                str, str, Optional[Callable[[float, Dict[str, Any]], Optional[float]]]
            ],
            ...,
        ] = (
            ...
            (
                "ws_home_avg_possession",
                "home_possession_style",
                lambda v, _: _clamp(v / 100.0 if v > 1 else v, 0.0, 1.0),
            ),
            (
                "ws_away_avg_possession",
                "away_possession_style",
                lambda v, _: _clamp(v / 100.0 if v > 1 else v, 0.0, 1.0),
            ),
            ...
            (
                "bf_market_margin_pct",
                "bookmaker_margin",
                lambda v, _: v / 100.0 if v > 1 else v,
            ),
        )
        ...
        for source, target, transform in overrides:
            ...
            if transform is not None:
                try:
                    ...
                    logger.debug(
                        "Enhanced feature transform failed for %s -> %s: %s",
                        source,
                        target,
                        exc,
                    )
    ...
    def _apply_enhanced_xg_consistency(
        ...
        def _consistency(
            recent: Optional[float], avg: Optional[float]
        ) -> Optional[float]:
    ...
    def _validate_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        ordered = features.reindex(
            columns=self.expected_columns, fill_value=fill_value
        ).copy()
        ...
        if ordered.isnull().any().any():
            if self._fail_closed_enabled():
                missing = [
                    str(col) for col in ordered.columns if ordered[col].isnull().any()
                ]
                raise DataUnavailableError(
