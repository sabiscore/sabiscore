...
class CanonicalFixture(Base):
    ...
    home_team_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("canonical_teams.id"), nullable=True
    )
...
class MarketSnapshot(Base):
    ...
    __table_args__ = (
        Index(
            "ix_market_snapshots_fixture_time", "canonical_fixture_id", "captured_at"
        ),
    ...
    home_implied_prob_devigged: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
    draw_implied_prob_devigged: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
    away_implied_prob_devigged: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
...
class UserPreference(Base):
    ...
    timezone: Mapped[str] = mapped_column(
        String, nullable=False, default="Africa/Lagos"
    )
    default_league: Mapped[str | None] = mapped_column(
        String, nullable=True, default="EPL"
    )
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)
...
class AnalyticsEvent(Base):
    ...
    client_platform: Mapped[str | None] = mapped_column(
        String, nullable=True, default="web"
    )
...
class UserNotificationLog(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "ix_notif_logs_anon_unread", "anonymous_session_id", "read", "created_at"
        ),
        {"extend_existing": True},
    )
