...
async def get_match_odds(
    ...
    try:
        ...
        if cached_result:
            ...
            return [
                OddsResponse(**item) if isinstance(item, dict) else item
        ...
        if not odds_snapshots:
            raise HTTPException(
                status_code=404, detail=f"No odds found for match {match_id}"
            )
...
@router.get(
    "/latest/{match_id}", response_model=Dict[str, OddsResponse], deprecated=True
)
async def get_latest_odds(
    ...
    try:
        ...
        subquery = (
            select(Odds.bookmaker, func.max(Odds.timestamp).label("latest_ts"))
            .where(Odds.match_id == match_id)
...
async def get_odds_movement(
    ...
    try:
        ...
        query = (
            select(Odds)
            .where(and_(Odds.match_id == match_id, Odds.timestamp >= cutoff_time))
        ...
        if not odds_history:
            raise HTTPException(
                status_code=404, detail=f"No odds history found for match {match_id}"
            )
...
async def get_best_line(
    ...
    try:
        ...
        if best_bookmaker is None:
            raise HTTPException(
                status_code=404, detail=f"No odds available for outcome: {outcome}"
            )
...
async def create_odds_snapshot(
    ...
    try:
        ...
        logger.info(
            "Created research-only legacy odds reference for match %s",
            odds_data.match_id,
        )
...
def _analyze_odds_movement(odds_history: List[Odds]) -> Dict:
    ...
    for bookmaker, snapshots in by_bookmaker.items():
        ...
        movements[bookmaker] = {
            "home_win": {
                ...
                "change": last.home_win - first.home_win
                if first.home_win and last.home_win
                else 0,
                "change_percent": (
                    (last.home_win - first.home_win) / first.home_win * 100
                )
                if first.home_win and last.home_win and first.home_win > 0
                else 0,
            },
            "draw": {
                "opening": first.draw,
                ...
                "change_percent": ((last.draw - first.draw) / first.draw * 100)
                if first.draw and last.draw and first.draw > 0
                else 0,
            },
            "away_win": {
                "opening": first.away_win,
                "current": last.away_win,
                "change": last.away_win - first.away_win
                if first.away_win and last.away_win
                else 0,
                "change_percent": (
                    (last.away_win - first.away_win) / first.away_win * 100
                )
                if first.away_win and last.away_win and first.away_win > 0
                else 0,
            },
            "snapshots_count": len(snapshots),
            "time_span_hours": (last.timestamp - first.timestamp).total_seconds() / 3600
            if last.timestamp and first.timestamp
            else 0,
        }
    ...
    return {
        ...
        "time_range_hours": (
            odds_history[-1].timestamp - odds_history[0].timestamp
        ).total_seconds()
        / 3600
