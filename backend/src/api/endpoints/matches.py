...
from ...schemas.match import MatchSummary, MatchListResponse, MatchDetailResponse
...
async def get_upcoming_matches(
    league: Optional[str] = Query(
        None, description="Filter by league (epl, bundesliga, laliga, etc)"
    ),
    days_ahead: int = Query(7, ge=1, le=30, description="Number of days to look ahead"),
    limit: int = Query(
        20, ge=1, le=100, description="Maximum number of matches to return"
    ),
    ...
    try:
            ...
            match_responses = [
                MatchSummary(
                    ...
                    match_date=datetime.fromisoformat(
                        m["match_date"].replace("Z", "+00:00")
                    )
                    if isinstance(m.get("match_date"), str) and m.get("match_date")
        ...
        if cached_result:
            ...
            return (
                MatchListResponse(**cached_result)
        ...
        if settings.mock_mode:
            ...
            mock_matches = mock_generator.generate_upcoming_matches(
                days=days_ahead, count=limit
            )

            # Filter by league if specified
            if league:
                mock_matches = [
                    m for m in mock_matches if m["league"].upper() == league.upper()
                ]
            ...
            logger.info(
                f"Generated {len(match_responses)} mock matches (league={league})"
            )
            return response
        ...
        odds_map = await _fetch_latest_odds(
            db, [str(match.id) for match in matches if match.id]
        )
        ...
        logger.info(
            f"Fetched {len(match_responses)} upcoming matches (league={league})"
        )
        return response
...
async def search_matches(
    ...
    try:
        ...
        for match in all_matches:
            home_name = (
                match.home_team.name if match.home_team else match.home_team_id or ""
            )
            away_name = (
                match.away_team.name if match.away_team else match.away_team_id or ""
            )
            match_league = match.league_id or ""

            # Filter by team name
            if (
                query_text.lower() not in home_name.lower()
                and query_text.lower() not in away_name.lower()
            ):
                continue
        ...
        logger.exception(
            "Match search failed", extra={"query": query_text, "league": league}
        )
...
async def get_match_detail(
    ...
    try:
        ...
        if cached_result:
            return (
                MatchDetailResponse(**cached_result)
                if isinstance(cached_result, dict)
                else cached_result
            )
...
async def get_matches_by_league(
    ...
    status: Optional[str] = Query(
        None, description="Filter by status (scheduled, live, finished)"
    ),
