...
async def oauth_password_form(username: str = Form(...), password: str = Form(...), scope: str = Form(default="")) -> OAuthPasswordForm:
...
async def _check_rate_limit(client_ip: str) -> None:
    async with _rate_lock:
        ...
        if len(recent) >= AUTH_RATE_LIMIT_REQUESTS:
            raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Too many authentication attempts. Please wait a moment.")
        recent.append(now)
...
async def _username_exists(db: AsyncSession, username: str) -> bool:
    result = await db.execute(select(UserAccount.id).where(UserAccount.username == username))
...
async def register_user(payload: UserCreate, request: Request, response: Response, db: AsyncSession = Depends(get_async_session)):
    ...
    user = UserAccount(email=email, username=username, full_name=payload.full_name or username, hashed_password=get_password_hash(payload.password), email_verified=False, is_active=True)
    db.add(user)
    try:
        await db.commit()
...
async def login_user(payload: LoginRequest, request: Request, response: Response, db: AsyncSession = Depends(get_async_session)):
    await _check_rate_limit(_client_ip(request))
    ...
    response.set_cookie(key="sabi_session", value=token, max_age=int(expires_delta.total_seconds()), httponly=True, secure=settings.app_env == "production", samesite="lax", path="/")
    anon_id = get_anon_id_from_request(request)
    if anon_id:
        try:
            await UserStateService.merge_anonymous_state(db, user_id=str(user.id), anonymous_session_id=anon_id)
            ...
            pass
    return LoginResponse(access_token=token, token_type="bearer", expires_in=int(expires_delta.total_seconds()), user=_serialize_user(user))


@auth_router.post("/oauth/google", response_model=LoginResponse)
async def login_with_google(payload: GoogleOAuthRequest, request: Request, response: Response, db: AsyncSession = Depends(get_async_session)):
    await _check_rate_limit(_client_ip(request))
    try:
    ...
    identity_result = await db.execute(select(UserIdentity).where(UserIdentity.provider == "google", UserIdentity.provider_subject == subject))
    identity = identity_result.scalar_one_or_none()
    user: Optional[UserAccount] = None
    if identity:
        user_result = await db.execute(select(UserAccount).where(UserAccount.id == identity.user_id))
        user = user_result.scalar_one_or_none()
    if user is None:
        user = await _get_user_by_email(db, email)
        if user is None:
            username = await _unique_username(db, email.split("@", 1)[0])
            user = UserAccount(email=email, username=username, full_name=claims.get("name") or username, avatar_url=claims.get("picture"), email_verified=True, hashed_password=None, is_active=True)
            db.add(user)
            await db.flush()
        elif not user.is_active:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Account disabled")
        if identity is None:
            identity = UserIdentity(user_id=str(user.id), provider="google", provider_subject=subject, provider_email=email)
            db.add(identity)
    if not user.is_active:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Account disabled")
    now = datetime.now(timezone.utc).replace(tzinfo=None)
...
async def cookie_login_user(payload: LoginRequest, request: Request, response: Response, db: AsyncSession = Depends(get_async_session)):
...
async def login_via_oauth_form(request: Request, form_data: OAuthPasswordForm = Depends(oauth_password_form), db: AsyncSession = Depends(get_async_session)):
    await _check_rate_limit(_client_ip(request))
...
async def merge_anonymous(payload: MergeAnonymousRequest, request: Request, current_user: UserAccount = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_session)):
    anon_id = payload.anonymous_session_id or get_anon_id_from_request(request)
    if not anon_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No anonymous session identifier provided or found in headers/cookies")
    result = await UserStateService.merge_anonymous_state(db, user_id=str(current_user.id), anonymous_session_id=anon_id)
    return MergeAnonymousResponse(status=result["status"], user_id=result["user_id"], anonymous_session_id=result["anonymous_session_id"], merged_favorites=result["merged_favorites"], merged_saved_matches=result["merged_saved_matches"])


@users_router.get("/favorites", response_model=List[FavoriteResponse])
async def list_favorites(request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    if not user and not anon_id:
        return []
    favs = await UserStateService.get_favorites(db, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None)
...
async def add_favorite(payload: FavoriteCreate, request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    if not user and not anon_id:
        anon_id = str(uuid.uuid4())
    fav = await UserStateService.add_favorite(db, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None, entity_type=payload.entity_type, entity_id=payload.entity_id)
    return FavoriteResponse.model_validate(fav)


@users_router.delete("/favorites/{favorite_id}")
async def delete_favorite_by_id(favorite_id: str, request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    if not user and not anon_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication or anonymous session required")
    success = await UserStateService.remove_favorite(db, favorite_id=favorite_id, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None)
    if not success:
...
async def delete_favorite_by_entity(entity_type: str, entity_id: str, request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    if not user and not anon_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication or anonymous session required")
    success = await UserStateService.remove_favorite(db, entity_type=entity_type, entity_id=entity_id, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None)
    return {"status": "DELETED" if success else "NOT_FOUND"}


@users_router.get("/saved-matches", response_model=List[SavedMatchResponse])
async def list_saved_matches(request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    if not user and not anon_id:
        return []
    matches = await UserStateService.get_saved_matches(db, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None)
...
async def save_match(payload: SavedMatchCreate, request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    if not user and not anon_id:
        anon_id = str(uuid.uuid4())
    saved = await UserStateService.save_match(db, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None, match_id=payload.match_id, target_outcome=payload.target_outcome, notes=payload.notes)
    return SavedMatchResponse.model_validate(saved)


@users_router.delete("/saved-matches/{match_id}")
async def remove_saved_match(match_id: str, request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    if not user and not anon_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication or anonymous session required")
    success = await UserStateService.remove_saved_match(db, match_id=match_id, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None)
    if not success:
...
async def get_user_preferences(request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    pref = await UserStateService.get_preferences(db, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None)
    return PreferenceResponse.model_validate(pref)


@users_router.put("/preferences", response_model=PreferenceResponse)
async def update_user_preferences(payload: PreferenceUpdate, request: Request, db: AsyncSession = Depends(get_async_session)):
    user = await get_optional_user_from_request(request, db)
    anon_id = get_anon_id_from_request(request)
    if not user and not anon_id:
        anon_id = str(uuid.uuid4())
    pref = await UserStateService.update_preferences(db, user_id=str(user.id) if user else None, anonymous_session_id=anon_id if not user else None, odds_format=payload.odds_format, timezone_pref=payload.timezone, default_league=payload.default_league)
    return PreferenceResponse.model_validate(pref)
