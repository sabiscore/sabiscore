...
async def create_prediction(
    ...
    try:
        ...
        # Generate prediction
        logger.info(
            f"Generating prediction for {request.home_team} vs {request.away_team}"
        )
        ...
        try:
            ...
            logger.error(
                "Prediction timed out for %s vs %s",
                request.home_team,
                request.away_team,
            )
            raise HTTPException(
        ...
        try:
            cache_backend.set(
                f"prediction:{match_identifier}", result.model_dump(), ttl=900
            )
        except Exception as cache_exc:
            logger.debug(
                "Failed to write alias cache for %s: %s", match_identifier, cache_exc
            )
        ...
        try:
            ...
            predicted_outcome = (
                max(preds, key=lambda outcome: float(preds[outcome])) if preds else None
            )
            edge_pct = None
            ...
            request_context.state.prediction_result = {
                ...
                if hasattr(result.league, "value")
                else str(result.league),
...
async def get_prediction(
    ...
    try:
        ...
        age_minutes = (
            datetime.now(timezone.utc) - prediction.created_at
        ).total_seconds() / 60
        if age_minutes > 60:
            raise HTTPException(
                status_code=410,
        ...
        if payload:
            if isinstance(payload, str):
                try:
                    ...
                    logger.warning(
                        "Failed to decode stored prediction payload for %s: %s",
                        match_id,
                        decode_err,
                    )
                    payload = None
            if isinstance(payload, dict):
                try:
                    ...
                    logger.warning(
                        "Stored prediction payload invalid for %s: %s",
                        match_id,
                        val_err,
                    )
                    payload = None
...
async def _save_prediction_to_db(
    db: AsyncSession, prediction_data: PredictionResponse, match_id: Optional[str]
):
