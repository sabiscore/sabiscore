...
if TYPE_CHECKING:
    ...
    from ..models.ensemble import (
        SabiScoreEnsemble,
    )  # pragma: no cover - only for type checking
...
def _http_error(status_code: int, detail: str, error_code: str) -> HTTPException:
    return HTTPException(
        ...
        detail=ErrorResponse(detail=detail, error_code=error_code).model_dump(
            mode="json"
        ),
    )
...
async def health_check(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        models_healthy = bool(
            model_instance and getattr(model_instance, "is_trained", False)
        )
        # indicate if a background model load is in progress
        model_loading = bool(
            getattr(request.app.state, "model_load_in_progress", False)
        )
        ...
        status = (
            "healthy"
            if all([db_healthy, cache_healthy, models_healthy])
            else "degraded"
        )
...
async def search_matches(
    ...
    try:
        ...
        logger.exception(
            "Match search failed", extra={"query": query, "league": league}
        )
...
async def generate_insights(
    ...
    try:
        ...
        if not body.matchup or not body.matchup.strip():
            raise _http_error(
                400,
                "Matchup parameter is required and cannot be empty",
                "INVALID_MATCHUP",
            )
        ...
        if model is None:
            logger.error(
                ...
                extra={"matchup": body.matchup, "league": league},
            )
            raise _http_error(
                503,
        ...
        response = (
            insights
            if isinstance(insights, InsightsResponse)
            else InsightsResponse(**insights)
        )
        try:
        ...
        raise _http_error(
            500, f"Insights validation error: {exc}", "INSIGHTS_VALIDATION_ERROR"
        ) from exc
    except ValueError as exc:
        # Handle validation errors from downstream components
        logger.warning(
            f"Validation error: {str(exc)}",
            extra={"matchup": body.matchup, "league": body.league},
        )
        raise _http_error(
            400, f"Invalid input: {str(exc)}", "VALIDATION_ERROR"
        ) from exc
        ...
        raise _http_error(
            500, "Failed to generate insights - internal server error", "INSIGHTS_ERROR"
        ) from exc
    ...
    try:
        ...
        if league:
            ...
            if not league_meta:
                raise _http_error(
                    404,
                    f"No metadata available for league '{league}'",
        ...
        raise _http_error(
            500, "Failed to get model status", "MODEL_STATUS_ERROR"
        ) from exc
