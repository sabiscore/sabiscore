...
async def check_and_broadcast_edges(
    match_id: str, odds_data: Dict, edge_detector: EdgeDetector
):
    ...
    try:
        ...
        if value_bet:
            ...
            logger.info(
                "Edge alert for match %s: %s%", match_id, value_bet["edge_percentage"]
            )
...
async def trigger_isr_revalidation(match_id: str):
    ...
    try:
        ...
        if not revalidate_secret:
            logger.warning(
                "ISR revalidation skipped because REVALIDATE_SECRET is not configured"
            )
            return

        # Construct revalidation payload
        payload = {"secret": revalidate_secret, "path": f"/match/{match_id}"}
