...
class AdvancedMetricsPayload(BaseModel):
    ...
    ppda_home: Optional[float] = Field(
        None, description="Home team PPDA (opponent passes / defensive actions)"
    )
    ppda_away: Optional[float] = Field(None, description="Away team PPDA")
    ppda_status: str = Field(
        MetricStatus.UNAVAILABLE.value, description="Status of PPDA metric"
    )

    psxg_home_delta: Optional[float] = Field(
        None,
        description="Home keeper PSxG - goals conceded (positive = saved > expected)",
    )
    psxg_away_delta: Optional[float] = Field(
        None, description="Away keeper PSxG - goals conceded"
    )
    psxg_status: str = Field(
        MetricStatus.UNAVAILABLE.value, description="Status of PSxG metric"
    )

    xt_status: str = Field(
        MetricStatus.ADVISORY_REQUIRES_CORPUS.value,
        description="Expected Threat status",
    )
    ...
    xt_reason: Optional[str] = Field(
        "Requires certified event-corpus pipeline", description="xT diagnostic note"
    )
...
class ModelIdentityPayload(BaseModel):
    ...
    feature_schema_version: str = Field(
        "phase7_68", description="Feature schema version"
    )
...
class DecisionStatePayload(BaseModel):
    ...
    research_only: bool = Field(
        True, description="Whether staking is prohibited due to uncertified state"
    )
