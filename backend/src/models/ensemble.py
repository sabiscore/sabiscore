...
class EnsembleModel:
    ...
    def build_ensemble(self, X: pd.DataFrame, y: pd.DataFrame) -> None:
        ...
        try:
            ...
            self._train_meta_model(
                meta_features_train, y_train, meta_features_test, y_test
            )
    ...
    def _create_meta_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for model_name, model in self.models.items():
            ...
            if probs.shape[1] > 2:
                meta_features[f"{model_name}_prob_home"] = probs[
                    :, 0
                ]  # Home win probability
                meta_features[f"{model_name}_prob_draw"] = probs[
                    :, 1
                ]  # Draw probability
                meta_features[f"{model_name}_prob_away"] = probs[
                    :, 2
    ...
    @classmethod
    def _repair_sklearn_compatibility(
        cls, obj: Any, seen: Optional[set[int]] = None
    ) -> None:
        ...
        if isinstance(obj, DecisionTreeClassifier) and not hasattr(
            obj, "monotonic_cst"
        ):
            obj.monotonic_cst = None

        if (
            obj.__class__.__module__.startswith("sklearn.")
            and getattr(obj, "n_jobs", None) == -1
        ):
            obj.n_jobs = 1
    ...
    def _train_meta_model(
        ...
        self.meta_model = CalibratedClassifierCV(
            estimator=base_lr, method="isotonic", cv=5
        )
    ...
    def _evaluate_ensemble(self, X_test: pd.DataFrame, y_test: pd.DataFrame) -> None:
        ...
        brier = 0.0
        for label_idx, prob_column in enumerate(
        ...
        logloss = log_loss(
            y_true,
            predictions[["home_win_prob", "draw_prob", "away_win_prob"]].values,
            labels=[0, 1, 2],
        )
    ...
    def load_model(cls, model_path: str) -> "SabiScoreEnsemble":
        ...
        try:
            ...
            instance.bivariate_poisson_overlay = model_data.get(
                "bivariate_poisson_overlay"
            )
            ...
            raise ModelLoadError(
                f"Failed to unpickle model file {model_path}: {e}"
            ) from e
        except Exception as e:
    ...
    def load_latest_model(cls, models_path: str) -> "SabiScoreEnsemble":
        ...
        if not os.path.exists(models_path) or not os.path.isdir(models_path):
            raise FileNotFoundError(
                f"Models path not found or is not a directory: {models_path}"
            )
        ...
        if os.path.exists(v2_model_path):
            try:
                ...
                logger.warning(
                    f"Failed to load V2 model: {e}, falling back to legacy models"
                )
        ...
        model_files.sort(
            key=lambda x: os.path.getmtime(os.path.join(models_path, x)), reverse=True
        )
        ...
        for candidate in model_files:
            ...
            if size < 10_240:  # 10KB
                logger.warning(
                    f"Skipping candidate {candidate} (size={size} bytes) - too small to be a valid model"
                )
                continue
            ...
            try:
                ...
                logger.exception(
                    f"Unexpected error loading model candidate {candidate}: {e}"
                )
                last_exc = e
    ...
    def _load_v2_model(cls, model_path: str) -> "EnsembleModel":
        ...
        try:
            ...
            instance.models = {
                "v2_ensemble": save_data.get("model")
            }  # Wrap in dict for compatibility
            instance.meta_model = save_data.get(
                "model"
            )  # V2 model acts as meta-learner
            ...
            raise ModelLoadError(
                f"Failed to load V2 model from {model_path}: {e}"
            ) from e
...
class LogOddsResidualWrapper(BaseEstimator):
    ...
    def fit(self, X, y, **kwargs):
        ...
        # XGBoost
        if (
            hasattr(self.base_estimator, "fit")
            and "xgboost" in type(self.base_estimator).__module__
        ):
            self.base_estimator.fit(X, y, base_margin=base_margin, **kwargs)
        # LightGBM
        elif (
            hasattr(self.base_estimator, "fit")
            and "lightgbm" in type(self.base_estimator).__module__
        ):
    ...
    def predict_proba(self, X):
        ...
        # XGBoost
        if (
            hasattr(self.base_estimator, "predict_proba")
            and "xgboost" in type(self.base_estimator).__module__
        ):
        ...
        # LightGBM
            ...
            hasattr(self.base_estimator, "predict")
            and "lightgbm" in type(self.base_estimator).__module__
        ):
