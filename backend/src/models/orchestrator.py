...
class ModelOrchestrator:
    ...
    def train_all_models(self, db: Session):
        ...
        for league_key, model in self.models.items():
            ...
            if len(matches) < 500:
                print(
                    f"⚠️  Insufficient data ({len(matches)} matches). Need 500+ for training."
                )
                continue
            ...
            self.redis.setex(
                f"model:{league_key}:metadata", 86400, json.dumps(metadata)
            )
    ...
    def _build_match_features(self, match, db: Session) -> Dict:
        ...
        # Home team stats
        features["home_goals_scored_l5"] = sum(
            m.home_score for m in home_recent if m.home_score
        )
        features["home_goals_conceded_l5"] = sum(
            m.away_score for m in home_recent if m.away_score
        )
        features["home_xg_l5"] = sum(m.home_xg for m in home_recent if m.home_xg) / max(
            len(home_recent), 1
        )
        features["home_xga_l5"] = sum(
            m.away_xg for m in home_recent if m.away_xg
        ) / max(len(home_recent), 1)

        # Away team stats
        features["away_goals_scored_l5"] = sum(
            m.away_score for m in away_recent if m.away_score
        )
        features["away_goals_conceded_l5"] = sum(
            m.home_score for m in away_recent if m.home_score
        )
        features["away_xg_l5"] = sum(m.away_xg for m in away_recent if m.away_xg) / max(
            len(away_recent), 1
        )
        features["away_xga_l5"] = sum(
            m.home_xg for m in away_recent if m.home_xg
        ...
        features["home_points_l5"] = sum(
            [3 if x == 1 else 1 if x == 0.5 else 0 for x in home_form]
        )
        features["away_points_l5"] = sum(
            [3 if x == 1 else 1 if x == 0.5 else 0 for x in away_form]
        )

        # Clean sheets
        features["home_clean_sheets_l5"] = sum(
            1 for m in home_recent if m.away_score == 0
        )
        features["away_clean_sheets_l5"] = sum(
            1 for m in away_recent if m.home_score == 0
        )
        ...
        h2h_matches = (
            ...
            .filter(
                and_(
                    ...
                    Match.match_date < match.match_date,
                    Match.match_date
                    >= match.match_date - timedelta(days=730),  # 2 years
                )
            )
            .order_by(Match.match_date.desc())
            .limit(5)
        ...
        h2h_home_wins = sum(
            ...
            if m.home_team == match.home_team and m.home_score > m.away_score
        )
        features["h2h_home_wins_l5"] = h2h_home_wins
        features["h2h_goals_per_game"] = sum(
            m.home_score + m.away_score for m in h2h_matches if m.home_score
        ) / max(len(h2h_matches), 1)
    ...
    def predict(
        ...
        try:
            ...
            return {"error": str(e), "league": league, "match_id": match_id}
    ...
    def _is_cache_fresh(self, cached: Dict) -> bool:
        ...
        return (
            datetime.now(timezone.utc).replace(tzinfo=None) - cached_time
    ...
    def _run_validation_suite(self, db: Session):
        ...
        for league_key, model in self.models.items():
            ...
            print(f"\n{league_key.upper()}:")
            print(
                f"  Accuracy: {accuracy:.1f}% (target: {self._get_accuracy_target(league_key)})"
            )
